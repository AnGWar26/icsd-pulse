...
def process_emp(week,sheet_name=0):
    ...
    if int(week) not in range(1,7):
        emp = pd.read_excel('../data/emp/employ1_week' + str(week) + '.xlsx', sheet_name=sheet_name, skip_rows=[0,1,2], header=[3,4], index_col=0, skipfooter=1, nafilter=False, keep_default_na=False)
    else:
        emp = pd.read_excel('../data/emp/employ1_week' + str(week) + '.xlsx', sheet_name=sheet_name, skip_rows=[0,1,2], header=[3,4], index_col=0, nafilter=False, keep_default_na=False)
